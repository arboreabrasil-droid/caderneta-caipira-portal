...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        setAceitouTermos(resultado.aceitouTermos); // ← NOVO

        setVerificando(false);
      } else {
        setAcesso(false);
        setPlano(null);
        setAceitouTermos(false); // ← NOVO
      }
  ...
  // ← NOVO: se tem acesso mas não aceitou, bloqueia aqui
  if (user && acesso && !aceitouTermos) {
    return <AceiteTermos user={user} onAceitou={() => setAceitouTermos(true)} />;
  }
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-8 lg:p-12 bg-white">
        <div className="w-full max-w-md">
          ...
          <p className="mt-6 text-center text-xs sm:text-sm text-gray-600">
            Não tem acesso? <span className="text-green-700 font-semibold">Solicite um convite no canal</span>
          </p>
